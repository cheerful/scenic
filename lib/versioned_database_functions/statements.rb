module VersionedDatabaseFunctions
  ...
  module Statements
    ...
    def create_view(name, version: nil, sql_definition: nil, materialized: false)
      ...
      if materialized
        VersionedDatabaseFunctions.database.create_materialized_view(name, sql_definition)
      else
        VersionedDatabaseFunctions.database.create_view(name, sql_definition)
      end
    end
    ...
    def drop_view(name, revert_to_version: nil, materialized: false)
      if materialized
        VersionedDatabaseFunctions.database.drop_materialized_view(name)
      else
        VersionedDatabaseFunctions.database.drop_view(name)
      end
    end
    ...
    def update_view(name, version: nil, sql_definition: nil, revert_to_version: nil, materialized: false)
      ...
      if materialized
        VersionedDatabaseFunctions.database.update_materialized_view(name, sql_definition)
      else
        VersionedDatabaseFunctions.database.update_view(name, sql_definition)
      end
    end
    ...
    def replace_view(name, version: nil, revert_to_version: nil, materialized: false)
      ...
      VersionedDatabaseFunctions.database.replace_view(name, sql_definition)
    end

    private

    def definition(name, version)
      VersionedDatabaseFunctions::Definition.new(name, version).to_sql
    end
  end
end
